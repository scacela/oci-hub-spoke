...
resource "oci_core_instance" "hub_compute" {
    ...
    create_vnic_details {
        ...
        display_name = "${var.deploy_network ? replace(replace(replace(replace(var.hub_sub_display_names[count.index%(var.add_subnet["hub"] ? 2 : 1)], var.network_name["hub"], var.compute_name["hub"]), local.subnet, local.compute_instance), "/[^A-Za-z0-9]/", ""), local.region, "") : replace(var.hub_sub_display_names[0], "/[^A-Za-z0-9]/", "")}${var.compute_num_nodes["hub"] > 1 ? format("-%s", floor(count.index/(var.add_subnet["hub"] ? 2 : 1))+1) : ""}-vnic"
        hostname_label = "${var.deploy_network ? replace(replace(replace(replace(var.hub_sub_display_names[count.index%(var.add_subnet["hub"] ? 2 : 1)], var.network_name["hub"], var.compute_name["hub"]), local.subnet, local.compute_instance), "/[^A-Za-z0-9]/", ""), local.region, "") : replace(var.hub_sub_display_names[0], "/[^A-Za-z0-9]/", "")}${var.compute_num_nodes["hub"] > 1 ? format("-%s", floor(count.index/(var.add_subnet["hub"] ? 2 : 1))+1) : ""}"
